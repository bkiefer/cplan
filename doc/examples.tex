\example A sub-node is replaced with fresh content. Notice the match
...
\example Selecting a specific relation, and adding to it
\begin{verbatim}
/// @aa:bb(<C>(<Mod>(:g ^ <x> y)))
<C>(<Mod>(#m:g ^ ! <Cont>)) -> #m ^ <Cont>(:new ^ clean).
\end{verbatim}

\example Add a default in case of feature absence
\begin{verbatim}
/// @aa:ascr(<C>d)
:ascr ^ !<Tense> -> # ^ <Tense>pres.
\end{verbatim}

\example Set a global variable as marker, and test in the second rule
\begin{verbatim}
...
\example type disjunction, variable \texttt{t} matching the whole node
...
\example Less preferable rewrite of the last example, the same variable
...
\example Using \tok{=>} rules to walk through a list
\begin{verbatim}
...
\example Randomizing with complex values
\begin{verbatim}
...
\example Alternative randomization, maybe not very convenient
\begin{verbatim}
...
\example Using global variable as node store.
...
\example Global variables can be used with path expressions to get
...
\example Bind values to global variables and use them in another rule
\begin{verbatim}
/// @d:dvp(<foo>(:a ^ <F>(b:c ^ d)))
...
\end{verbatim}
 ...
 \example Be careful that you use bound variables correctly! If you use
...
\example Check for structural equality (will also succeed if coreferent)
\begin{verbatim}
/// @d:dvp(<Content>(:bar ^ baz) ^ <Wh-Restr>(:bar ^ baz))
:dvp ^ <Content>#c: ^ <Wh-Restr>#c: -> # ^ :equals.
\end{verbatim}

\example Check for identity (will only succeed if coreferent)
\begin{verbatim}
...
( eq(#x, 1) ~ 1 )
->
###part1 = random("La prima domanda ט: ",
...
\example Non-integer numbers must be passed to functions as strings. Boolean functions
return zero or one.
\begin{verbatim}
...
\example You can use arbitrary encodings in your grammar files,
