...
public class DagToLF {
  ...
  static short[] postponedFeatures = null;
  ...
  private static Element handleContent(Element diamond, DagNode sub,
    ...
    if (coreferences.containsKey(sub)) {
      ...
      if (relMode == null ||
        ...
        List<DagEdge> postponed =
          new ArrayList<DagEdge>(postponedFeatures.length);
        // if there are features and relations, deal with them properly
        //----------------------------------------------
        while (edge != null) {
          short featId = edge.getFeature();
          for (int ppfeat :postponedFeatures) {
            if (featId == ppfeat) {
              postponed.add(edge);
              featId = -1;
              break;
  ...
  /**
    ...
    try {
	    if (lfp.parse()) {
	      result = lfp.getResultLF();
	    } else {
	      throw new IllegalArgumentException(
	          lex.getLastErrorPosition().toString()+" "+lf.toString());
	    }
